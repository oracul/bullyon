...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/bullyon.conf are parsed in qt/bitcoin.cpp's main()
...
{
    uiInterface.ThreadSafeMessageBox(str, _("bullyon"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("bullyon"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 62124 or testnet: 62125)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 62123)") + "\n" +
...
bool AppInit2()
{
    ...
    // bullyon: Keep irc seeding on by default for now.
//    if (fTestNet)
//    {
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  bullyon is probably already running."), GetDataDir().string().c_str()));
